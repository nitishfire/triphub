...
export default function AdminHotelBookings() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/admin/hotels/' + hotelId + '/bookings'),
  ...
  const totalRevenue = bookings.reduce((sum, b) => sum + (b.pricePerNight || 0), 0);

  return (
    <div className="min-h-screen bg-slate-50 pt-20 relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-72 bg-gradient-to-br from-emerald-900 via-teal-900 to-cyan-900 z-0 rounded-b-[3rem] shadow-2xl" />
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0 overflow-hidden">
        <div className="absolute -top-24 -left-24 w-96 h-96 bg-emerald-500/20 rounded-full blur-3xl" />
        <div className="absolute top-10 -right-24 w-80 h-80 bg-teal-500/20 rounded-full blur-3xl" />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="pt-6 pb-16 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-emerald-200 text-xs font-bold uppercase tracking-widest mb-1">Hotel Booking History</p>
              <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">{hotelName || hotelId}</h1>
              <p className="text-emerald-200 text-sm mt-0.5">{bookings.length} booking{bookings.length !== 1 ? 's' : ''} found</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-3">
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl px-7 py-4 text-right border border-white/20">
              <p className="text-emerald-200 text-xs font-bold uppercase tracking-wider">Total Revenue</p>
              <p className="text-2xl font-black text-white mt-1">{'\u20B9'}{totalRevenue.toLocaleString()}</p>
            </div>
            <Link to={'/admin/hotels/' + hotelId + '/rooms'} className="flex items-center gap-2 bg-white/10 backdrop-blur-xl hover:bg-white/20 text-white font-bold px-5 py-2.5 rounded-2xl border border-white/20 transition-all text-sm">
        ...
        <div className="-mt-8 pb-12">
          {bookings.length === 0 ? (
            <div className="bg-white rounded-3xl p-16 text-center shadow-xl border border-gray-100">
              ...
              <p className="text-gray-500 mt-2">This hotel has no bookings on record</p>
            </div>
          ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {[
                            ...
                            { label: 'Duration', value: b.availableDays + ' days' },
